...
class PromptIR(nn.Module):
    def __init__(
        ...
        if self.decoder:
            self.prompt1 = PromptGenBlock(
                prompt_dim=64, prompt_len=5, prompt_size=64, lin_dim=2 * dim
            )
            self.prompt2 = PromptGenBlock(
                prompt_dim=224 - 2 * dim, prompt_len=5, prompt_size=32, lin_dim=4 * dim
            )
            self.prompt3 = PromptGenBlock(
                prompt_dim=512 - 4 * dim, prompt_len=5, prompt_size=16, lin_dim=8 * dim
            )
        ...
        # )
        self.reduce_chan_level3 = nn.Conv2d(
            6 * dim, int(dim * 2**2), kernel_size=1, bias=bias
        )
        ...
        self.head3 = nn.Conv2d(int(dim * 2**2), out_channels, kernel_size=1, bias=bias)
        self.head2 = nn.Conv2d(int(dim * 2**1), out_channels, kernel_size=1, bias=bias)
    ...
    def forward(self, inp_img, noise_emb=None):
        ...
        y3 = self.head3(out_dec_level3)
        y3_up = F.interpolate(y3, size=(H, W), mode="bilinear", align_corners=False)

        y2 = self.head2(out_dec_level2)
        y2_up = F.interpolate(y2, size=(H, W), mode="bilinear", align_corners=False)

        out_dec_level1 = self.output(out_dec_level1) + inp_img

        return out_dec_level1, y2_up, y3_up
